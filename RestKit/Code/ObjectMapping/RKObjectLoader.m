...
- (BOOL)isResponseMappable {
    ...
    if ([self.response isFailure]) {
        ...
        RKLogWarning(@"Encountered unexpected response with status code: %ld (MIME Type: %@ -> URL: %@ -> Body %@)", (long) self.response.statusCode, self.response.MIMEType, self.URL, [self.response bodyAsString]);
