...
- (void)closeConnection:(SimpleHTTPConnection *)connection;
{
	...
	for(k = 0; k < [requests count]; k++) {
		SimpleHTTPRequest *request = [requests objectAtIndex:k];
...
- (void)processNextRequestIfNecessary
{
    if( [self currentRequest] == nil && [requests count] > 0 ) {
		...
		if([[currentRequest method] isEqualToString:@"POST"]) {
			response = [delegate processPOST:currentRequest];
		} else {
			response = [delegate processGET:currentRequest];
		}
...
- (void)processResponse:(SimpleHTTPResponse *)response
{
    ...
    if([response content]) {
        NSString *length = [NSString stringWithFormat:@"%d", [[response content] length]];
        CFHTTPMessageSetHeaderFieldValue(msg, (CFStringRef)@"Content-Length", (CFStringRef)length);
        CFHTTPMessageSetBody(msg, (CFDataRef)[response content]);
    }
